...
*/
{
  ...
  if(iplot >0 )
  {
    ...
    if(ifiled)
      for(i=1;i<=iplot;i++)
        fprintf(ifiled,"%-5s %-3d %-3d %-9.6f %-10d %-9.6f %-6.3f\n",
...
/*Print out the values requested.*/
    for(i=ilo;i<=my_min(ihi,iplot);i++)
    {
      sprintf(out," %-8s %3d %3d %10.6f %10d %10.6f %8.3f %-12s %8s\n",cplot[i],lplot[i],nplot[i],tplot[i],noplot[i],fplot[i],aplot[i],machine[MACHINE],"C   ");
      if (lchop)
      {
      if (!langshow)  chop(out,70,78);
      if (!mshow)  chop(out,57,69);
